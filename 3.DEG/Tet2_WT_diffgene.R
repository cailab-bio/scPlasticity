...
library(ggplot2)
library(dplyr)
# setwd("D:/KS椤圭洰/鍏紬鍙锋枃绔?/鍫嗗彔鏌辩姸鍥炬樉绀烘瘮渚?")
...
color_cluster=c("#af2934","#ffe327","#2f4e87","#b0b9b8","#f0eedf",
                "#aed4e9","#f4a69a","#3ba889","#4593c3","#f18e0c",
                "#262a35","#c5942e","#a2a7ab") #棰滆壊鍙互鑷繁瀹氫箟锛実gsci鍜孯ColorBrewer鍖呯殑棰滆壊鎼厤灏卞緢NICE浜?
...
p1<-jjVolcano(diffData = clusterdeg_1,
              base_size = 20,
              log2FC.cutoff = 0.5, # logfc鏍规嵁鑷繁鎯呭喌璁惧畾
              pvalue.cutoff = 0.05,
              size  = 3, #璁剧疆鐐圭殑澶у皬
              #aesCol = c('blue','orange'), #璁剧疆鐐圭殑棰滆壊
              tile.col = ident.colors, #璁剧疆cluster鐨勯鑹?
              #col.type = "adjustP", #璁剧疆鍥句緥鏄剧ず鏂瑰紡
              topGeneN = 5, #璁剧疆灞曠ずtopN鐨勫熀鍥?
              flip = F,
